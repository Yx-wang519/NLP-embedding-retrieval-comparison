...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        for i, cat in enumerate(categories):
            cat_embed = averaged_glove_embeddings_gdrive(
        ...
        ##########################################
        # build / refresh category embeddings cache if needed
        need_refresh = (cache_key not in st.session_state) or (
            set(st.session_state.get(cache_key, {}).keys()) != set(categories)
        )
        if need_refresh:
            get_category_embeddings(embeddings_metadata)
        ...
        sent_embed = get_sentence_transformer_embeddings(
